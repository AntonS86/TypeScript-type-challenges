...
type GetSign<S extends string> = S extends `${infer M}${string}` ? M extends '+'| '-' ? M : '' : '';
type a = GetSign<'+'>
//   ^?

type GetPercent<S extends string> = S extends `${string}%` ? '%' : ''
type b = GetPercent<'+100%'>
//   ^?

type GetNum<S extends string> = S extends `${GetSign<S>}${infer M}${GetPercent<S>}` ? M : '';

type PercentageParser<A extends string> = [GetSign<A>, GetNum<A>, GetPercent<A>]
type m = PercentageParser<'+100%'>
//   ^?
